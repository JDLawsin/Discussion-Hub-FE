import { BookOpen, MessageSquare, Star, Tag } from "lucide-react";
import { ProtocolHit } from "../types/types";
...
const ProtocolCard = ({ hit }: Props) => (
  <Link href={`/protocol/${hit.id}`}>
      ...
      <div className="flex">
        <div className="flex-1 p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="flex items-center gap-1 text-xs font-medium text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-400">
            ...
            <span className="flex items-center gap-1">
              <StarRating rating={hit.average_rating} />
              <span className="ml-1">{hit.average_rating?.toFixed(1)}</span>
            </span>
          </div>
        </div>
      </div>
