      ...
      <div className="bg-white border border-gray-200 rounded-xl hover:border-orange-200 hover:shadow-md transition-all duration-200 cursor-pointer group">
        <div className="flex">
          <div className="flex-1 p-4">
            ...
            <h3 className="text-sm font-semibold text-gray-900 mb-1.5 group-hover:text-orange-600 transition-colors line-clamp-2">
              {thread.title}
            </h3>
            ...
            {thread.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {thread.tags.map((tag) => (
                  <span
                    key={tag.name}
                    className="text-xs text-gray-500 bg-gray-100 hover:bg-orange-50 hover:text-orange-500 px-2 py-0.5 rounded-full transition-colors"
                  >
                    #{tag.name}
                  </span>
                ))}
