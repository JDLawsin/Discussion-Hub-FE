...
const TagInput = ({ tags, onAdd, onRemove, error, maxTags = 5 }: Props) => {
  ...
  return (
    <div className="space-y-1.5">
      <div
        className={`flex flex-wrap gap-1.5 min-h-10 px-3 py-2 bg-gray-50 border rounded-xl transition-all focus-within:bg-white focus-within:border-orange-300 focus-within:ring-2 focus-within:ring-orange-100 ${
      ...
      >
        {tags.map((tag) => (
          <span
            key={tag}
            className="flex items-center gap-1 text-xs font-medium text-orange-600 bg-orange-100 px-2 py-0.5 rounded-full"
          >
